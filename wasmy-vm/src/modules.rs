...
use wasmy_abi::{CODE_EXPORTS, CodeMsg, Result};
    ...
    for function in module.exports().functions() {
        ...
        if name == WasmHandlerApi::onload_symbol() {
            ...
            if ty.params().len() > 0 || ty.results().len() > 0 {
                return CodeMsg::result(CODE_EXPORTS, format!("Incompatible Export Type: fn {}(){{}}", WasmHandlerApi::onload_symbol()));
            }
            continue;
        }
        WasmHandlerApi::symbol_to_method(name).map_or_else(|| {
            ...
            if ty.results().len() == 0 && ty.params().eq(&[Type::I32, Type::I32]) {
                #[cfg(debug_assertions)]println!("module exports wasmy function: {:?}", function);
                Ok(())
            } else {
                return CodeMsg::result(CODE_EXPORTS, format!("Incompatible Export Type: {:?}", function));
            }
        })?;
    }
